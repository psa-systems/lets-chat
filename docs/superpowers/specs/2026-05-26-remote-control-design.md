...
LC-185 as built (`desktop/src/inject.rs` + `desktop/src/main.rs`):
- **Bridge.** A Tauri `initialization_script` (runs at document start, before the server page's scripts) adds the three `lc:control-*` listeners and forwards to native via `window.__TAURI_INTERNALS__.invoke` (the internal entrypoint, so `withGlobalTauri` stays off and the page gets no broad Tauri API surface). Two commands: `rc_session(active)` and `rc_input(frame)`.
- **Remote-IPC capability.** App commands are unreachable from a remote (non-`tauri://`) origin unless a capability with a matching `remote.urls` entry allows them. The server URL is user-configurable, so a static capability file can't cover it; instead `main.rs` adds a runtime `CapabilityBuilder` (`.local(false).remote("scheme://host[:port]/*").permission("allow-rc-session"/"allow-rc-input")`) scoped to exactly the configured origin. Declaring the app commands in `build.rs` (`AppManifest::commands([...])`) is what generates the `allow-*` permissions; it also flips app commands to ACL-checked for local pages, so `set_server_url` is declared + granted `allow-set-server-url` in `capabilities/default.json`. The injector additionally re-asserts `active` (set only by the local user's Grant) before injecting, so a peer flooding the channel with no live grant injects nothing. This is no weaker than the existing `LETS_CHAT_SERVER_URL` trust posture (a compromised server page can already drive the IPC bridge); it is NOT a defense against a malicious server.
- **Windows synthesis.** `SendInput`. Mouse uses `MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_VIRTUALDESK` with `[0,1] -> 0..=65535` over the virtual desktop (DPI/resolution-independent, no `GetSystemMetrics` needed). Keyboard is scan-code (`KEYEVENTF_SCANCODE`, `KEYEVENTF_EXTENDEDKEY` for nav/right-modifier keys) via a `KeyboardEvent.code -> PS/2 set-1` table; modifier *state* is not consulted because the controller sends discrete down/up for the modifier keys themselves. Held keys/buttons are tracked and force-released on every disarm (stuck-key prevention). Non-Windows targets are `#[cfg(not(windows))]` no-ops.

Documented limits (LC-185, not defeated):
- **UIPI.** `SendInput` cannot drive a higher-integrity (elevated/admin) window; those clicks/keys are silently dropped by Windows. An admin-elevated window on the controlled host is out of the controller's reach.
- **Single shared surface.** `[0,1]` is mapped over the whole virtual desktop, which matches a primary / full-screen share. Per-monitor surface selection (sharing one of several monitors and mapping precisely onto it) is out of scope (already listed under "Out of scope").
- **Wheel granularity.** Web wheel delta is treated as ~100px per notch when converting to `WHEEL_DELTA`; scroll speed is approximate.
...
LC-186 as built (kill-switch / auto-revoke / audit / limits):
- **Kill-switch.** Two always-reachable severs for the sharer: a "Stop control" button on a persistent red banner (`data-lc-control-banner`, shown the whole time a peer holds control), and a **global OS hotkey** `CommandOrControl+Alt+F9` registered natively via `tauri-plugin-global-shortcut`. The hotkey is the reliable path when the controller holds the input focus (clicking the button is hard while a remote cursor moves). Its handler flips the native injector off + releases held input *directly* (`ControlState::kill`, independent of the webview), then `window.eval`s a `lc:control-kill` DOM event so the page signals the peer `revoke` and drops the banner.
- **Auto-revoke** (`call.js` `revokeAsSharer`): on call end (teardown), screen-share stop (`endScreenShare`), and data-channel drop (`controlChannel.onclose/onerror`). Each both signals `revoke` (so the controller stops capturing + the audit row closes) and disarms locally. The earlier-design "focus loss to a higher-integrity window" auto-revoke is NOT implemented - UIPI already blocks injection into elevated windows, so the exposure it targets does not exist; the controller instead sees a standing UIPI hint (`data-lc-control-uipi`).
- **Audit.** `chat.db` `remote_control_sessions` (migration 0052) via `db::remote_control_audit`. `relay_control_signal` opens a row on `grant` (controller = grant recipient, sharer = granter) and closes it on `revoke`; a WS-disconnect backstop (`end_sessions_for_user`) closes any session a hard drop left open. Content is never stored - only participant ids + timestamps.
- **Limits.** `RateLimitKind::RemoteControlRequest` caps `request` signals at 10/min per requester (over-limit drops silently, same posture as the verified/block gate). The block-list + verified-email gate from LC-183 still apply. **Decision: no max-session-duration / periodic re-consent in v1** - the kill-switch + auto-revoke + disconnect-close already bound exposure, and periodic re-consent adds friction without a matching threat once an instant sever exists. Revisit if a concrete need appears.
