...
| 22 | routes_uploads.rs::preview_query_returns_smaller_bytes_than_original | saas only | assertion (500 vs 200) | Resolved in Task 6: NOT a saas-specific bug. Test passes in isolation in saas (`cargo test --test routes_uploads -- preview_query_returns_smaller_bytes_than_original`); full `routes_uploads` binary passes 9/9 in saas. The 500 was the same intermittent flake that affects `routes_uploads::upload_happy_path` and `::send_message_with_attachment` in standalone under concurrent-binary load. Same root cause: filesystem contention across upload-aware test binaries running in parallel. Flagged as a flake in the "Tests that fail for non-drift reasons" section; out of scope for phase 24. | flake (same family as standalone routes_uploads flake) |
...
5. **flake** (3 tests, all in `routes_uploads.rs`): `upload_happy_path_returns_file_id_and_url`, `send_message_with_attachment_renders_inline_image` (standalone), and `preview_query_returns_smaller_bytes_than_original` (saas) all fail intermittently with 500 when the full test suite runs concurrently. Pass cleanly in isolation. Confirmed via Task 6 investigation that the saas-mode case is the same root cause as the standalone cases — concurrent-binary filesystem contention in the upload pipeline, not a saas-specific bug. Per-PID tempdirs should prevent path collision; the contention is at a different layer (likely shared on-disk state in the data dir or a singleton in the image pipeline). Out of scope here; flag for a follow-up phase if it shows up in CI noise.
...
- **Task 6 - Investigate `preview_query_returns_smaller_bytes_than_original` saas-only 500. (DONE)** Reran in isolation: test passed. Reran the full `routes_uploads` binary in saas: 9/9 passed. Reran the full saas suite: 0 failures. Resolution: the saas-only 500 was the same intermittent flake observed in standalone `routes_uploads` tests during Task 3 verification. No code change needed. Reclassified to "flake" in the audit table.
