...
**DM chip-rendering: option A (chip everywhere).** When a user types `@here` in a DM, the body renderer chip-renders it as if it were a regular room message. The chip is text styling - no notification rows are written and no broadcast events fire (the DM gate in the resolver path skips broadcast resolution for `room_type == "dm"`), so the chip in the DM body is purely a visual treatment of the token. Alternative: thread room-context into the renderer so it can detect DMs and emit plain text. Rejected because (a) the renderer is currently context-free and adding a room-type parameter to every call site is a much larger blast radius than the user-visible payoff, and (b) the chip-without-effect is honest about what the author typed even if it had no recipients. A DM is implicitly a one-on-one conversation and "you typed @here at one person" is unsurprising to look at. Pinned here so this decision doesn't drift during implementation.
