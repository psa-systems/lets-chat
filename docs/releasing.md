...
- **Desktop**: a self-updating Tao+Wry app. `publish-release.yml` fires on a `v*` tag push, cross-builds the Linux/Windows binaries, and uploads them plus a `latest.json` manifest to Forgejo Generic Packages. The app's updater reads `latest.json` to discover the newest version.
