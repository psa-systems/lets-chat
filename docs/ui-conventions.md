...
Why id-keyed OOB regions instead of swapping the whole `#sidebar` or re-rendering the page: htmx silently drops an OOB swap whose id is absent from the current DOM, so the fragment lands **only** on connections actually viewing that region and is a no-op everywhere else. Scope ids that vary by entity (enclave room nav -> `#sidebar-nav-{enclave_id}`) so one enclave's update can't swap into another's.

That drop-if-absent property covers the *rendering*, not the decisions taken before it, so it does not by itself make the server page-agnostic. Two paths already read the connection's page: `render_sidebar` needs `current_enclave` to pick the sidebar's shape (LC-337), because `#sidebar` exists on every page and a wrong-shape render is a successful swap, not a dropped one; and `render_new_message_or_bump` treats a subscribed room as open in the foreground, so it advances the read watermark, clears mentions and broadcasts a `DmRead` receipt. A stale subscription is therefore not inert. LC-834 is what keeps that state honest: the client sends a `page_context` frame (room and enclave, `null` included) on connect and on every navigation, and `ws.rs::apply_page_context` rebuilds the connection's page-scoped state to exactly what a socket freshly opened on that page would hold. A page whose live topic is enclave-scoped needs no extra work - the frame is derived from the same `data-lc-live-room` / `data-lc-live-topic` attributes step 3 already asks for. The same frame is where the connection re-reads its user record (LC-838, `ws.rs::refresh_account`): one read per page move, so a role or preference change reaches a live socket on the user's next move, as it did when every move opened a fresh socket. Both loops read a per-frame / per-event snapshot of that record; never cache a field of it across frames.

The sidebar is the one part of the shell a `#main` swap leaves behind, and LC-836 keeps it current from the same frame. `apply_page_context` reports whether the frame MOVED the connection (a previous frame existed and named a different room or enclave; the connect frame and LC-318's same-url re-assert are not moves, so a full page load gets no redundant swap of the sidebar it just rendered), the receive loop sends `ChatEvent::PageChanged` to that one connection via `Hub::send_to_conn` (never `broadcast_to_user`: the user's other tabs did not move), and the send task answers it with the same whole-sidebar `render_sidebar` OOB path mark-all-read uses. That path now takes the page's room as well as its enclave and marks the active row through `routes::mark_sidebar_active`, the single helper the room and DM page handlers also call, so a navigation without a page load highlights exactly the row a full render would. The row the page GET just marked read is cleared in the same render; its `DmRead` alone would not reach the navigating tab, which was not yet subscribed to the destination when the GET ran.
...
The boundary is stated, not implied. `ci-build/check-swap-safe-scripts.nu` (`just check-swap-safe-scripts`, and a step of the Check workflow) carries a `SHELL` allowlist of the templates that render exactly once per page load and are therefore exempt: `base.html`, `layout.html`, `partials/theme_bootstrap.html`, and the eight `*_modal.html` partials `layout.html` includes after `</main>`. Everything else under `server/templates/` must declare, so a new template is covered by default rather than by remembering to add it; `server/templates/email/` is out of scope as it is for every other template rule, because a mail client runs no scripts. The guard also rejects a `<script src=...>` outside the shell, since a bundle re-fetched and re-executed on every swap double-registers everything it defines.
...
**Confirmation text is attribute text, never a literal in a handler (LC-753, LC-771).** Askama escapes every `{{ }}` through `askama_escape`, which maps `'` to `&#x27;`, and the HTML parser decodes that back to a bare `'` before an event-handler attribute is compiled as JavaScript. Any apostrophe inside `onclick="return confirm('...')"` therefore ends the JS string literal early, the handler never compiles, and the destructive action runs on the first click with no dialog and no error the operator can see. LC-753 hit it from catalog strings (`admin-webhooks-rotate-confirm`, "Rotate this webhook's..."); LC-771 from runtime data (a member display name such as `O'Brien` in the enclave transfer and kick confirmations, which no charset check prevents). The fix is structural, not textual: the text lives in a `data-lc-confirm` attribute on the same element as the handler, and the handler reads it back, `onclick="return confirm(this.getAttribute('data-lc-confirm'))"` (or `onsubmit=`, whichever event carries the guard today; see the LC-739 rule above for which one that is). Attribute text is decoded by the HTML parser and handed to JavaScript as a value, so it never meets the JS parser and any character round-trips. Composed strings (prefix, runtime value, suffix) compose inside the attribute value exactly as they did inside the literal. `hx-confirm="{{ "key"|t }}"` is unaffected for the same reason: htmx reads the attribute as text.

- `ci-build/check-confirm-apostrophe.nu` (wired into `just check` and the Check workflow) rejects any `confirm(` followed by a JS string delimiter anywhere in `server/templates/**/*.html`, any inline handler that calls `confirm(` with anything other than `this.getAttribute('data-lc-confirm')`, and any canonical call on an element whose opening tag has no `data-lc-confirm` attribute (the dialog would show `null`). It fails if no canonical site exists at all, so it cannot pass vacuously. Email templates are excluded: they render in a mail client with no scripting.
- Keep `data-lc-confirm` on the element that carries the handler, so `this.getAttribute` finds it, and keep the opening tag's first line starting with `<tag` when the attributes span several lines: that is how the guard finds the element's extent.
...
## Data tables (LC-510, LC-737, LC-745, LC-756)
...
- `check-table-shape.nu` has no exemption list: every table under `server/templates/` outside the email templates is checked. The last two exemptions, the cohort-retention heat grid (`admin/analytics.html`) and the IMAP ingress drop log (`admin/settings.html`), moved onto the component in LC-756; both sit in their own `card lc-table-wrap` inside the section card that already held them, and `.lc-table` is column-count agnostic, so the retention grid's variable week columns need nothing extra. The component gallery (`dev/theme_gallery.html`) is not exempt either: LC-744 put it on the production markup, because a gallery that demonstrates a different contract from the one the real tables use is worse than no gallery.
...
`manifest.webmanifest` and `offline.html` cannot interpolate the version as templates do, so LC-794 serves them from routes (`routes/assets.rs`) that substitute `__ASSET_VERSION__` the same way `routes/push.rs` does for `sw.js`. That lets the icon family (`favicon.svg`, `icon-192.png`, `icon-512.png`, `icon-maskable-512.png`) carry `?v=` too, so the immutable header now covers the whole directory. Two references stay unversioned by design: the dev theme gallery (it has no `asset_version` in scope) and the web-push notification icon (`src/push/payload.rs`, built with no request context and fetched once per notification). `server/tests/lc776_asset_cache.rs` asserts the header behaviour and fails on any new unversioned reference outside its allowlist.
