...
async fn setup_pool() -> SqlitePool {
    ...
    let migration16 = include_str!("../migrations/auth/0016_sso_identities.sql");
    sqlx::raw_sql(migration16)
        .execute(&pool)
        .await
        .expect("auth migration 16");

    let migration17 = include_str!("../migrations/auth/0017_sso_providers.sql");
    sqlx::raw_sql(migration17)
        .execute(&pool)
        .await
        .expect("auth migration 17");

    let migration18 = include_str!("../migrations/auth/0018_sso_flows_provider.sql");
    sqlx::raw_sql(migration18)
        .execute(&pool)
        .await
        .expect("auth migration 18");
    let migration19 = include_str!("../migrations/auth/0019_sso_group_mappings.sql");
    sqlx::raw_sql(migration19)
        .execute(&pool)
        .await
        .expect("auth migration 19");

    let migration20 = include_str!("../migrations/auth/0020_session_tenant.sql");
