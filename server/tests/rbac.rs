...
async fn setup_pool() -> SqlitePool {
    ...
    let migration24 = include_str!("../migrations/auth/0024_user_locale.sql");
    sqlx::raw_sql(migration24)
        .execute(&pool)
        .await
        .expect("auth migration 24");
    let migration25 = include_str!("../migrations/auth/0025_user_theme.sql");
    sqlx::raw_sql(migration25)
        .execute(&pool)
        .await
        .expect("auth migration 25");
    let migration26 = include_str!("../migrations/auth/0026_user_density.sql");
    sqlx::raw_sql(migration26)
        .execute(&pool)
        .await
        .expect("auth migration 26");
    pool
}
