...
async fn setup_pools() -> (SqlitePool, SqlitePool) {
    ...
    let auth_m6 = include_str!("../migrations/auth/0006_user_blocks.sql");
    sqlx::raw_sql(auth_m6)
        .execute(&auth_pool)
        .await
    ...
    let auth_m8 = include_str!("../migrations/auth/0008_two_factor.sql");
    sqlx::raw_sql(auth_m8)
        .execute(&auth_pool)
        .await
    ...
    let auth_m10 = include_str!("../migrations/auth/0010_password_reset.sql");
    sqlx::raw_sql(auth_m10)
        .execute(&auth_pool)
        .await
        .expect("auth migration 10");
    let auth_m11 = include_str!("../migrations/auth/0011_email_verification.sql");
    sqlx::raw_sql(auth_m11)
        .execute(&auth_pool)
        .await
        .expect("auth migration 11");
    let auth_m12 = include_str!("../migrations/auth/0012_session_metadata.sql");
    sqlx::raw_sql(auth_m12)
        .execute(&auth_pool)
        .await
        .expect("auth migration 12");

    let auth_m13 = include_str!("../migrations/auth/0013_digest_columns.sql");
    sqlx::raw_sql(auth_m13)
        .execute(&auth_pool)
        .await
        .expect("auth migration 13");

    let auth_m14 = include_str!("../migrations/auth/0014_login_alerts.sql");
    sqlx::raw_sql(auth_m14)
        .execute(&auth_pool)
        .await
    ...
    let chat_m14 = include_str!("../migrations/chat/0014_mentions.sql");
    sqlx::raw_sql(chat_m14)
        .execute(&chat_pool)
        .await
