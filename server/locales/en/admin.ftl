...
admin-bots-assistant-desc = Choose which bot the AI assistant posts as: the identity shown on /ask answers and catch-up summaries. Defaults to the built-in "assistant" bot.
...
admin-webhooks-rotate-confirm = Rotate the signing secret for this webhook? The current secret stops working immediately.
...
admin-settings-llm-audience-label = Who can use AI
admin-settings-llm-audience-everyone = Everyone
admin-settings-llm-audience-everyone-note = All members see and use the AI features in rooms and DMs they belong to.
admin-settings-llm-audience-staff = Staff only
admin-settings-llm-audience-staff-note = Limit to site admins, enclave owners/admins and room moderators. Useful for piloting before opening it up.
...
# LC-712: AI help desk documentation knowledge base.
admin-settings-helpdocs-heading = AI help desk (documentation)
admin-settings-helpdocs-unconfigured-title = No embeddings endpoint configured.
admin-settings-helpdocs-unconfigured-body = The docs search stays empty until an embeddings endpoint is set via
admin-settings-helpdocs-note = The /support assistant answers from the product documentation indexed below. Add one source per line as "product | docs-index-URL"; the indexer crawls that page's sub-pages, splits them on headings, and embeds them. Save your changes, then Reindex (or wait for the periodic refresh).
admin-settings-helpdocs-sources-label = Documentation sources
admin-settings-helpdocs-sources-note = One per line: product name, a pipe, then the docs index URL. Example: mokosh-server | https://a8n.systems/apps/mokosh-server/docs
admin-settings-helpdocs-save = Save sources
admin-settings-helpdocs-index-label = Indexed
admin-settings-helpdocs-index-chunks = chunks
admin-settings-helpdocs-index-products = products
admin-settings-helpdocs-reindex = Reindex now
admin-settings-helpdocs-reindex-started = Reindex started; this runs in the background.
