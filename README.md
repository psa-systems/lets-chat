...
| `LETS_CHAT_STT_URL` / `_API_KEY` / `_MODEL` / `_PROMPT` | (none) | Optional **server-side** call transcription. Point `_URL` at an OpenAI-compatible `/v1/audio/transcriptions` endpoint (whisper.cpp server, faster-whisper, LocalAI, ...) to transcribe calls server-side instead of with the in-browser Web Speech engine: browser-agnostic (Firefox/Safari) and keeps audio off third-party clouds. `_API_KEY` is an optional bearer token; `_MODEL` defaults to `whisper-1`. `_PROMPT` (LC-591) is an optional glossary/style hint sent to the engine to bias spelling of names and jargon (a single global string). The server also requests `response_format=verbose_json` for real caption timestamps and sends the speaker's preferred locale as a `language` hint when set; engines that ignore either field degrade cleanly. Unset `_URL` = the in-browser engine. The endpoint is operator-trusted and **not** SSRF-filtered, so it may be `localhost`/internal; never point it at an untrusted host. |
