...
| `LETS_CHAT_UPDATE_URL` | `https://dev.a8n.run/api/packages/psa-systems/generic/lets-chat` | Forgejo Generic Packages root the desktop self-updater reads. Release builds compile in the path CI actually published to; this default applies to unstamped builds. Every fetch and each redirect hop is validated against a public-IP SSRF filter. |
