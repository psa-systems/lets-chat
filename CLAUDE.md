...
CI enforces this mechanically (LC-515): `ci-build/check-migration-immutability.nu`, wired into `.forgejo/workflows/check.yml`, fails any PR (and push to `main`) that modifies, renames, or deletes a `server/migrations/**` file already on `main`; adding a new migration passes. Review discipline alone had already failed here and on mokosh-server prod (DEV-395), so the gate is not optional. The check needs the checkout's `fetch-depth: 0` to see the merge-base with `origin/main`.
...
| `LETS_CHAT_LIVEKIT_URL` / `_API_KEY` / `_API_SECRET` | (none) | LC-512 stage audio (the SFU media for the LC-494 large-audience stage). All three required to enable; unset = stage mode still has roles + request-to-speak but no audio. `_URL` is the browser-facing LiveKit signaling URL (e.g. `wss://livekit.example.com`); the operator self-hosts a LiveKit server (Docker). Read on demand by `livekit::LiveKitConfig::from_env` (no AppState client, like the GIF picker). The server only mints LiveKit access tokens: `GET /room/{id}/stage/token` returns a JWT (HS256, signed with `_API_SECRET`) whose `video` grant pins the room (`stage-{id}`) and derives publish rights from the live stage roster - speakers/hosts get `canPublish`, listeners `canSubscribe` only - so a promote/demote re-mints with new rights (the client re-fetches). No Rust media SDK. The browser uses the `livekit-client` SDK, vendored same-origin at build time by `just vendor-js` (pinned, gitignored, no CDN at runtime, no CSP change - `connect-src wss:` already covers signaling). |
| `LETS_CHAT_STT_URL` / `_API_KEY` / `_MODEL` | (none) | LC-393 Phase 3 server-side call transcription. Set `_URL` to an OpenAI-compatible `/v1/audio/transcriptions` endpoint (whisper.cpp server, faster-whisper, LocalAI, ...) to switch call transcription from the in-browser Web Speech engine to server-side STT: browsers then capture short audio clips and `POST /call/transcript/{id}/audio` forwards each to the endpoint. Browser-agnostic (Firefox/Safari) and keeps audio off third-party clouds. `_API_KEY` (optional) is sent as a bearer token; `_MODEL` (optional) defaults to `whisper-1`. Unset = browser engine (Phases 1-2 behavior). Read by `stt::SttConfig::from_env`; the endpoint is operator-trusted and NOT SSRF-filtered (so it can be `localhost`/internal, like SMTP/IMAP) - never point it at an untrusted host. The transcription engine is advertised to clients via `GET /call/config` (`sttServer`). LC-483: when set, this endpoint ALSO transcribes uploaded voice messages (audio attachment + waveform) off the request path (`routes::room::maybe_transcribe_voice_message`); the stored transcript renders under the waveform and a `VoiceTranscribed` WS event re-renders the message live. So enabling STT now sends both call audio and voice-message audio to the endpoint (load/cost note for metered engines). LC-496: the same path ALSO transcribes recorded async video clips - the recorded container (`video/webm`/`video/mp4`) is forwarded to the endpoint as-is and an OpenAI-compatible engine demuxes the audio track itself (no server-side ffmpeg); the same `maybe_transcribe_voice_message` gate and `VoiceTranscribed` re-render cover clips. So enabling STT now additionally sends video-clip audio to the endpoint (extra load/cost + content note for metered or third-party engines; an STT engine that rejects video containers simply yields no clip transcript and the clip still posts/plays). |
| `LETS_CHAT_LLM_URL` / `_API_KEY` / `_MODEL` | (none) | LC-396 transcript AI summaries. Set `_URL` to an OpenAI-compatible `/v1/chat/completions` endpoint (Ollama, llama.cpp server, vLLM, LocalAI, ...) to enable the "Summarize" action on the transcript page, which sends the transcript text and stores the returned markdown summary + action items (cached in `call_transcripts.summary`). `_API_KEY` (optional) is a bearer token; `_MODEL` (optional) defaults to `gpt-4o-mini`. Unset = the action is hidden. Read by `llm::LlmConfig::from_env`; reached through the same operator-trusted, NOT-SSRF-filtered `http_client::outbound_trusted_post` path as STT (so a localhost engine works) - never point it at an untrusted host. Gated server-side via `AppState::llm_available`. LC-484: the same endpoint also powers the "Catch me up" / "Summarize" actions on rooms and threads (`routes::summary`), which send recent/unread or thread message text to the endpoint (no persistent cache - generated on demand). So enabling the LLM now sends chat message text in addition to transcripts (load/cost + content note for metered or third-party engines). LC-486: the same endpoint also powers per-message "Translate" (`routes::translate`), which sends a single message's text on demand and caches the result in `chat.db::message_translations` (keyed by message + locale, invalidated on edit). LC-492: the same endpoint also powers the in-channel AI assistant `/ask` (`routes::assistant`), which on each ask sends retrieved room context (FTS-matched messages + the room wiki/description) plus the question and posts the answer as the `assistant` bot. It is off by default and per-room opt-in (`rooms.assistant_enabled`, toggled on the room manage page) and additionally requires the LLM to be configured, so enabling the LLM does not by itself send anything new until a room manager turns the assistant on (content/cost note for metered or third-party engines). |
| `LETS_CHAT_GIPHY_API_KEY` / `_RATING` | (none) | LC-488 GIF picker (LC-505: migrated off the shut-down Tenor API to Giphy). Set `_API_KEY` to a Giphy API key to enable the composer GIF button: the server proxies search to Giphy (`GET /api/gifs`, trending when the query is empty), and on pick fetches the chosen GIF from the Giphy CDN, re-encodes it through the uploads pipeline, and posts it as a normal same-origin attachment (`POST /room/{id}/gif`) - GIFs are never hotlinked into messages. `_RATING` (optional) is `g`/`pg`/`pg-13`/`r` (default `pg-13`). Unset = the GIF button is hidden. Read on demand by `gif::GifConfig::from_env` (no AppState client). The fetch is confined to the Giphy CDN by a host allowlist (`gif::is_giphy_media_url`) on top of the `http_client` public-IP SSRF filter. Sends the user's search terms + posts to Giphy (third-party content/usage note). |
| `LETS_CHAT_BUNYIP_SSO_ISSUER` / `_CLIENT_ID` / `_CLIENT_SECRET` / `_REDIRECT_URI` | (none) | LC-22 pure-RP Bunyip SSO. All four are MANDATORY at startup; the standalone build refuses to boot without them (no `_ENABLED` flag, no local-auth fallback). `_REDIRECT_URI` must be `{base}/auth/bunyip/callback` and match a `redirect_uris` row on the OP's `oauth_clients`. Read by `oidc::BunyipSsoConfig::from_env`; discovery + JWKS are fetched server-to-server at startup and must succeed. |
| `LETS_CHAT_BUNYIP_SSO_INSECURE_TLS` | (unset = strict) | DEV ONLY. Set to `1`/`true` to make the SSO reqwest client `danger_accept_invalid_certs`, for a dev OP behind a self-signed cert (e.g. the Traefik default cert on `*.a8n.run`). reqwest's `rustls-tls` uses bundled webpki roots that ignore the system CA store, so there is no env/mount-only remedy; this is the escape hatch. NEVER set in production: it disables issuer TLS authentication for the whole SSO trust path. Read by `oidc::client::initialize`. |
| `LETS_CHAT_UPDATE_URL` | `https://dev.a8n.run/api/packages/a8n-tools/generic/lets-chat` | Forgejo Generic Packages root the desktop self-updater reads. The updater fetches `${URL}/latest/latest.json` for the manifest (and `${URL}/latest/latest.json.sig` for its signature) and downloads platform binaries from `${URL}/${version}/lets-chat-desktop-{linux,windows}-x86_64[.exe]`. Override to test against a fork or a local fixture. LC-210: every fetch (and each redirect hop) is validated against a public-IP SSRF filter, so a `http://127.0.0.1:...` local fixture needs `LETS_CHAT_UPDATE_URL_ALLOW_PRIVATE=1`. LC-210-BINARY-INTEGRITY: the manifest is Ed25519-signed and each binary's SHA-256 is verified before `self_replace`; a fixture mirror must serve a `latest.json.sig` over the manifest, and the binary must be built with the matching `LETS_CHAT_UPDATE_PUBLIC_KEY`. |
| `LETS_CHAT_UPDATE_PUBLIC_KEY` | (build-time; unset = updater fails closed) | LC-210-BINARY-INTEGRITY (#277). **Build-time** env (read via `option_env!` in `desktop/src/update_verify.rs`), NOT a runtime server var. Hex Ed25519 public key embedded into the desktop binary; the updater verifies the signed `latest.json` against it and refuses to self-update if empty. Matching private key is the CI release secret `DESKTOP_UPDATE_SIGNING_KEY`. Full provisioning + key-rotation runbook in `docs/desktop-update-signing.md`. |
| `LETS_CHAT_UPDATE_URL_ALLOW_PRIVATE` | (unset = disabled) | LC-210 desktop self-updater. Set to `1`/`true` to exempt the INITIAL `LETS_CHAT_UPDATE_URL` from the public-IP filter, for an operator running a private internal update mirror (or a loopback test fixture). Redirect targets are STILL validated regardless: a compromised mirror that 302s to an internal host is refused. Read per fetch. |
| `LETS_CHAT_RETENTION_SWEEP_ENABLED` | (unset = disabled) | Gates `spawn_message_retention_sweeper`. Set to `1` or `true` to enable the destructive hard-delete sweep that enforces per-room `retention_days`. Off by default while the strict-vs-loose semantics question for thread retention is open with the ticket author; current behavior is loosest-correct (sweep-by-newest-reply preserves active threads). Flipping requires a server restart; the spawn function reads the var at startup and does not poll. LC-207-OBSERVABILITY (#278): each completed tick is persisted to `chat.db::retention_sweep_status` (singleton) and surfaced read-only on `/admin/settings` (last run, last/total deleted, run count, last error) so an operator can confirm the destructive sweep ran without container logs. |
| `LETS_CHAT_BRIDGE_AVATAR_PROXY_ENABLED` | (unset = enabled) | LC-78-AVATAR-PROXY. Default-on. When enabled, a bridge daemon may submit a `foreign_avatar` URL; the server fetches it once, sniffs + re-encodes through the uploads pipeline, and serves it same-origin. Set to `false`/`0`/`no`/`off` to restore v1's reject-non-null posture (`POST /api/v1/bridges/{id}/messages` 400s on any non-null `foreign_avatar`). Read per request by `bridge_avatar::proxy_enabled`. |

## Email ingress (LC-77)

`spawn_email_poll` polls an operator-configured IMAP mailbox every 5 minutes; messages addressed to `<token>@<ingress-domain>` post to their room as the `MessageActor::EmailInbox` synthetic actor. See `docs/email-ingress.md` for the operator deployment guide, threat model, and failure-log taxonomy. Gated at startup on `LETS_CHAT_SECRET_KEY` set + `imap_inbox_config.enabled = 1` + `imap_inbox_config.ingress_domain` set; flipping `enabled` requires a server restart (same precedent as the retention sweeper). The IMAP password is AES-256-GCM-sealed at rest under `LETS_CHAT_SECRET_KEY` via `db::imap_config`; per-room inbox secrets are HMAC-SHA256-hashed via `db::email_inbox`. The named link-filter skip decision (mirroring the LC-74 webhook posture) is anchored in a code comment in `routes::room::finalize_email_inbox_message_send`. LC-207-OBSERVABILITY (#278): poll-loop health (`settings.db::imap_poll_status`, singleton, written every tick) and dropped-message reasons (`chat.db::email_ingress_drops`, swept at 30 days by the orphan sweeper alongside the dedup table) are surfaced read-only on `/admin/settings`, so "is the poll alive / why didn't my email post?" no longer needs container logs.
...
## Image decoders need explicit `Limits` (LC-206)

Every image decode in `uploads::pipeline` MUST set explicit `image::Limits` (`decode_limits()`: 256 MiB `max_alloc` + 16384 dimension caps), not rely on the `image`-crate default. Two reasons: the default is uneven (`ImageReader` defaults `max_alloc` to 512 MiB, but `GifDecoder::new` starts at `no_limits()` - the GIF path was unbounded), and a version bump can change it silently (a stale spike comment once asserted "image 0.25 has no default Limits", which was false). The byte caps (10 MiB upload / 1 MiB avatar) bound the INPUT file, not the decoded pixel buffer, so a small high-ratio file is a decompression bomb at decode time; only `Limits` catches that. Decode runs under `spawn_blocking` behind a 4-permit semaphore, so without `Limits` the worst case was 4 x 512 MiB+. The LC-206 grep-ban (`lc206_no_raw_image_decode_in_src`) already forces all decoders through `pipeline.rs`; keep `decode_limits()` applied at each site there. The valid-decompression-bomb regression lives in `uploads_pipeline.rs` (`png_decompression_bomb_rejected_by_limits`).
...
**Desktop changes additionally run `just test-desktop`** (LC-210). The desktop crate is bin-only, so its tests are `#[cfg(test)]` modules in `desktop/src/`, and `just test` / `just test-saas` are server-only - they do NOT run desktop tests. A PR touching `desktop/` that skips `just test-desktop` lands untested the way fmt slipped past `just check` on the LC-59 merge. `just check` only compiles desktop; it does not run its tests.
