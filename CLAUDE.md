...
### Per-message notification emails (LC-77-REPLY stage 1, #201)

`crate::email::notification::dispatch_mention_notification` fires fire-and-forget from the 5 mention-reconcile / DM-send hook sites in `routes::room`. Per-user opt-in (`users.notify_email_activity_enabled`, default 0), per-recipient rate cap (20/min via `RateLimitKind::EmailMentionNotification`), 7-day reply-token TTL in `chat.db::reply_tokens` (swept by `spawn_orphan_sweeper`). Outbound mail carries `Reply-To: reply-<token>@<ingress-domain>` (when ingress is configured) plus `Auto-Submitted: auto-generated` to break reciprocal loops. The inbound reply path (stage 2) is in flight - replies hit the polled mailbox today but drop with `address_no_match` until the resolver branch ships.
